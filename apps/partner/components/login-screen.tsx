...
export function LoginScreen({
  ...
  const partnerName = workspaceSlug ? PARTNER_LOGIN_NAMES[workspaceSlug] : null
        ...
        <CardHeader className="gap-5 border-b pb-6">
          ...
          <div className="space-y-2">
            <h1 className="font-heading text-2xl leading-snug font-medium">
              {staffOnly
                ? "Beam Partner staff"
                : (partnerName ?? "Beam Partner")}
            </h1>
            <p className="text-sm leading-6 text-muted-foreground">
              {staffOnly
                ? "Use your Beam Google account to access the global partner administration panel."
                : partnerName
                  ? `Sign in to the ${partnerName} partner workspace. Partners need a named invitation.`
                  : "Take Beam to a client without opening Beam Core. Staff sign in with Google. Partners use a named invitation."}
            </p>
          </div>
        </CardHeader>
