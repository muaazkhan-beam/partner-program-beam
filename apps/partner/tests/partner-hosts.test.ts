...
test("auth allowlist covers canonical, partner, local, and preview hosts", () => {
  ...
  assert.equal(canonicalSiteUrl(undefined), "https://partner.beam.ai")
  assert.equal(
    canonicalSiteUrl(" https://preview.example "),
    "https://preview.example",
  )
})

test("partner subdomains are classified separately from apex and unknown hosts", () => {
  assert.deepEqual(classifyPartnerHost("pwc-me.partner.beam.ai"), {
    ok: true,
    kind: "partner",
    slug: "pwc-me",
  })
  assert.deepEqual(classifyPartnerHost("roboyo.partner.beam.ai:443"), {
    ok: true,
    kind: "partner",
    slug: "roboyo",
  })
  assert.deepEqual(classifyPartnerHost("partner.beam.ai"), {
    ok: true,
    kind: "apex",
  })
  assert.deepEqual(
  ...
  assert.deepEqual(classifyPartnerHost("partners.beam.ai"), { ok: false })
})
